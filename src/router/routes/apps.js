export default [
  ...
  },
  {
    ...
    beforeEnter(to, _, next) {
      if (['important', 'completed', 'deleted'].includes(to.params.filter)) { next() } else next({ name: 'error-404' })
    },
  },
  {
    ...
    beforeEnter(to, _, next) {
      if (['team', 'low', 'medium', 'high', 'update'].includes(to.params.tag)) { next() } else next({ name: 'error-404' })
    },
  },
