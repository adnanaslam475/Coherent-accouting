        ...
        <b-col cols="10">
          <b-row class="mx-auto d-flex align-items-center justify-content-center">
                ...
                <b-list-group class="list-group-circle text-left">
                  <b-list-group-item v-for="(data, index) in pricing.basicPlan.planBenefits" :key="index">
                    <div v-if="data === 'companiess'">
                      {{ pricing.basicPlan.companyLimit }} {{ $t('companiess') }}
                    </div>
                    <div v-else-if="data === 'unlimited_ocr_invoices'">
                      {{ pricing.basicPlan.documentLimit }} {{ $t('unlimited_ocr_invoices') }}
                    </div>
                    <div v-else>
                      {{ $t(data) }}
                    </div>
              ...
              <b-card class="pb-1 popular" align="center">
                ...
                <b-list-group class="list-group-circle text-left">
                  <b-list-group-item v-for="(data, index) in pricing.beginnerPlan.planBenefits" :key="index">
                    <div v-if="data === 'companiess'"> {{ pricing.beginnerPlan.companyLimit }} {{ $t('companiess') }}
                    </div>
                    <div v-else-if="data === 'unlimited_ocr_invoices'">{{ pricing.beginnerPlan.documentLimit }} {{
                      $t('unlimited_ocr_invoices') }}</div>
            ...
            <b-col md="4">
              <b-card align="center" class="pb-1">
                <!-- img -->
                <b-img v-if="aeroplanImage" :src="aeroplanImage" class="mb-2 w-25" alt="enterprise svg img" />
                <!--/ img -->
                ...
                <b-list-group v-for="(data, index) in pricing.primaryPlan.planBenefits" :key="index"
                  ...
                  <b-list-group-item>
                    <div v-if="data === 'companiess'">{{ pricing.primaryPlan.companyLimit }} {{ $t('companiess') }}</div>
                    <div v-else-if="data === 'unlimited_ocr_invoices'">{{ pricing.primaryPlan.documentLimit }} {{
                      $t('unlimited_ocr_invoices') }}</div>
                    <div v-else>{{ $t('pricing.' + data) }}</div>
                  </b-list-group-item>
                </b-list-group>
            ...
            <b-col md="4">
              <b-card align="center" class="pb-1">
                ...
                <b-list-group v-for="(data, index) in pricing.platinumPlan.planBenefits" :key="index"
                  class="list-group-circle text-left">
                  <b-list-group-item>
                    <div v-if="data === 'companiess'"> {{ pricing.platinumPlan.companyLimit }} {{ $t('companiess') }}
                    </div>
                    <div v-else-if="data === 'unlimited_ocr_invoices'">{{ pricing.platinumPlan.documentLimit }} {{
                      $t('unlimited_ocr_invoices') }}</div>
                    <div v-else>{{ $t('pricing.' + data) }}</div>
                  </b-list-group-item>
                </b-list-group>
    ...
    return {
      ...
      pricing: {
        basicPlan: {
          ...
          popular: false,
          companyLimit: null, // Will be updated from the server
          documentLimit: null, // Will be updated from the server
        },
        beginnerPlan: {
          title: 'Beginner',
      ...
      useJwt.getPlansPrices().then((response) => {
        ...
        for (let i = 0; i < this.plansPrice.length; i++) {
          if (this.plansPrice[i].name === "BASIC") {
            this.pricing.basicPlan.companyLimit = this.plansPrice[i].compnayLimit;
            this.pricing.basicPlan.documentLimit = this.plansPrice[i].documentLimit;
          ...
          if (this.plansPrice[i].name === "BEGINNER") {
            this.pricing.beginnerPlan.companyLimit = this.plansPrice[i].compnayLimit;
            this.pricing.beginnerPlan.documentLimit = this.plansPrice[i].documentLimit;
            if (this.plansPrice[i].planType === "MONTHLY") {
          ...
          if (this.plansPrice[i].name === "STARTER") {
            this.pricing.starterPlan.companyLimit = this.plansPrice[i].compnayLimit;
            this.pricing.starterPlan.documentLimit = this.plansPrice[i].documentLimit;
            if (this.plansPrice[i].planType === "MONTHLY") {
          ...
          if (this.plansPrice[i].name === "ENTERPRISE") {
            this.pricing.primaryPlan.companyLimit = this.plansPrice[i].compnayLimit;
            this.pricing.primaryPlan.documentLimit = this.plansPrice[i].documentLimit;
            if (this.plansPrice[i].planType === "MONTHLY") {
          ...
          if (this.plansPrice[i].name === "PLATINIUM") {
            this.pricing.platinumPlan.companyLimit = this.plansPrice[i].compnayLimit;
            this.pricing.platinumPlan.documentLimit = this.plansPrice[i].documentLimit;
            if (this.plansPrice[i].planType === "MONTHLY") {
