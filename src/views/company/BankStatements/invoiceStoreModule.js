    ...
    fetchInvoices(ctx, queryParams) {
      ...
      if (queryParams.q || queryParams.dateFrom || queryParams.dateTo) {
        ...
        return new Promise((resolve, reject) => {
          axios
            .post(`/account/api/bank-statement/search/${companyId}/${pageNumber ? pageNumber : 1}/${perPageValue ? perPageValue : 10}`, data, config)
            .then((response) => {
              resolve(response)
            })
            .catch((error) => {
        ...
        return new Promise((resolve, reject) => {
          axios
            .get(`/account/api/bank-statement/list/${companyId}/${pageNumber ? pageNumber : 1}/${perPageValue ? perPageValue : 10}`, config)
            .then((response) => {
              resolve(response)
            })
            .catch((error) => {
